--! Previous: sha1:166b90c6bab14dd33465d74ebc7b4d5abc4e162e
--! Hash: sha1:63aee76fd8ba0e71d9167e3d6b1fcb06fda844ac
...
DROP POLICY IF EXISTS delete_self ON app_public.users;
...
create or replace function app_private.tg_users__make_first_user_admin() returns trigger as $$
begin
...
create or replace function app_public.current_user_is_admin() returns bool as $$
...
create table if not exists app_private.user_secrets (
...
DROP TRIGGER IF EXISTS _500_insert_secrets ON app_public.users;  
...
create table if not exists app_public.user_emails (
  id serial primary key,
...
create unique index IF NOT EXISTS uniq_user_emails_verified_email on app_public.user_emails(email) where is_verified is true;
alter table app_public.user_emails enable row level security;

DROP TRIGGER IF EXISTS _100_timestamps ON app_public.user_emails;  
create trigger _100_timestamps
  before insert or update on app_public.user_emails
  for each row
  execute procedure app_private.tg__update_timestamps();

DROP TRIGGER IF EXISTS _900_send_verification_email ON app_public.user_emails;
...
create table if not exists app_private.user_email_secrets (
...
DROP TRIGGER IF EXISTS _500_insert_secrets ON app_public.user_emails;
create trigger _500_insert_secrets
  after insert on app_public.user_emails
  for each row
...
create table if not exists app_public.user_authentications (
  id serial primary key,
...
DROP TRIGGER IF EXISTS _100_timestamps on app_public.user_authentications;
create trigger _100_timestamps
...
DROP POLICY IF EXISTS select_own ON app_public.user_authentications;
DROP POLICY IF EXISTS delete_own ON app_public.user_authentications;
...
create table if not exists app_private.user_authentication_secrets (
...
create or replace function app_public.forgot_password(email text) returns boolean as $$
declare
...
create or replace function app_private.login(username text, password text) returns app_public.users as $$
declare
...
create or replace function app_public.reset_password(user_id int, reset_token text, new_password text) returns app_public.users as $$
declare
  v_user app_public.users;
  v_user_secret app_private.user_secrets;
  v_reset_max_duration interval = interval '3 days';
...
create or replace function app_private.really_create_user(username text, email text, email_is_verified bool, name text, avatar_url text, password text default null) returns app_public.users as $$
declare
  v_user app_public.users;
...
create or replace function app_private.register_user(f_service character varying, f_identifier character varying, f_profile json, f_auth_details json, f_email_is_verified boolean default false) returns app_public.users as $$
declare
  v_user app_public.users;
  v_email citext;
  v_name text;
