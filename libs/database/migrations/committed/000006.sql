--! Previous: sha1:63aee76fd8ba0e71d9167e3d6b1fcb06fda844ac
--! Hash: sha1:1ea79be5f7a408f90c485a908dcd299ce8099976

-- Enter migration here

create table if not exists app_public.forums (
...
DROP TRIGGER IF EXISTS _100_timestamps ON app_public.forums;
...
DROP POLICY IF EXISTS select_all ON app_public.forums;
DROP POLICY IF EXISTS insert_admin ON app_public.forums;
DROP POLICY IF EXISTS update_admin ON app_public.forums;
DROP POLICY IF EXISTS delete_admin ON app_public.forums;
...
create table if not exists app_public.topics (
  id serial primary key,
...
DROP TRIGGER IF EXISTS _100_timestamps ON app_public.topics;
create trigger _100_timestamps
...
DROP POLICY IF EXISTS select_all ON app_public.topics;
DROP POLICY IF EXISTS insert_admin ON app_public.topics;
DROP POLICY IF EXISTS update_admin ON app_public.topics;
DROP POLICY IF EXISTS delete_admin ON app_public.topics;
...
create or replace function app_public.topics_body_summary(
  t app_public.topics,
  max_length int = 30
)
returns text
...
create table if not exists app_public.posts (
  id serial primary key,
...
DROP TRIGGER IF EXISTS _100_timestamps ON app_public.posts;
create trigger _100_timestamps
...
DROP POLICY IF EXISTS select_all ON app_public.posts;
DROP POLICY IF EXISTS insert_admin ON app_public.posts;
...
create or replace function app_public.random_number() returns int
language sql stable
as $$
  select 4;
$$;
...
create or replace function app_public.forums_about_cats() returns setof app_public.forums
language sql stable
as $$
